...
describe("Kelime Patlat tahta kuralları", () => {
  ...
  it("uzun kelimelere daha yüksek puan çarpanı verir ve botu insanî aralıkta bekletir", () => {
    ...
    expect(botThinkDelayMs(4, 4, () => 0)).toBeGreaterThanOrEqual(2_500);
    expect(botThinkDelayMs(4, 4, () => 0.99)).toBeLessThanOrEqual(16_000);
    expect(botThinkDelayMs(6, 4, () => 0)).toBeGreaterThanOrEqual(2_500);
    expect(botThinkDelayMs(8, 6, () => 0.5)).toBeGreaterThanOrEqual(2_500);
