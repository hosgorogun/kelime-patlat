...
export function SoloChallenge({ level, theme = "general", variationSeed, daily = false, excludeWords = [], onExit, onComplete, onNext }: { level: number; theme?: WordTheme; variationSeed?: number; daily?: boolean; excludeWords?: string[]; onExit: () => void; onComplete: (level: number, foundWords: string[]) => void; onNext: () => void }) {
  ...
  useEffect(() => {
    ...
    setRadarUsed(false); setRadarHighlights(new Set()); setTimeBonusText(null);
  ...
  return <ScrollView contentContainerStyle={[styles.content, { backgroundColor: activeTheme.background }]} scrollEnabled={!isSelecting} showsVerticalScrollIndicator={false}>
    ...
    <View style={[styles.found, { backgroundColor: activeTheme.trayBackground, borderColor: activeTheme.cellBorder }]}><Text style={[styles.foundLabel, { color: activeTheme.headerText }]}>BULDUKLARIN</Text><View style={styles.tags}>{found.length ? found.map((word) => <View key={word} style={[styles.tag, { backgroundColor: activeTheme.surface }]}><Text style={styles.tagText}>{word}</Text></View>) : <Text style={styles.empty}>İlk kelimeyi bul.</Text>}</View></View>
