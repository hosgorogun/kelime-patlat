...
export function SeasonHub({ playerId, progress, leaderboard, onBack, onSelectTheme }: { playerId: string; progress: PlayerProgress; leaderboard: LeaderboardEntry[]; onBack: () => void; onSelectTheme: (theme: ThemePackId) => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.packStack}>{THEME_PACKS.map((pack) => {
      const active = progress.selectedTheme === pack.id;
      return <Pressable key={pack.id} onPress={() => onSelectTheme(pack.id)} style={({ pressed }) => [styles.pack, active && { borderColor: pack.accent, backgroundColor: pack.glow }, pressed && styles.pressed]}><View style={[styles.packGlyph, { borderColor: pack.accent }]}><Text style={[styles.packGlyphText, { color: pack.accent }]}>{pack.icon}</Text></View><View style={styles.packCopy}><Text style={[styles.packLabel, { color: pack.accent }]}>{pack.label} {active ? "· ETKİN" : ""}</Text><Text style={styles.packTitle}>{pack.title}</Text><Text style={styles.packBody}>{pack.description}</Text></View><Text style={[styles.packArrow, { color: pack.accent }]}>{active ? "✓" : "→"}</Text></Pressable>;
    })}</View>
  </ScrollView>;
}
